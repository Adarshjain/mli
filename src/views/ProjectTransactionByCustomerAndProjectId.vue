...
import {
  execQuery,
  getProjectByIdQuery,
  getTransactionByProjectsIdAndCustomerIdQuery
} from "@/queries";
...
export default class TransactionPerCustomerPerProject extends Vue {
  ...
  async mounted() {
    const transactions = await execQuery(getTransactionByProjectsIdAndCustomerIdQuery(this.$route.params.id, this.$route.params.projectId))
    let balance = 0;
