      ...
      <template v-for="column in columns">
        <b-table-column :key="column.id" v-bind="column">
          ...
          <template v-slot="props">
            <NuxtLink v-if="column.field === 'project_name'" :to="`${$route.params.id}/projects/${props.row['project_id']}`" >
...
export default class TransactionPerCustomer extends Vue {
  async asyncData({params, $axios}) {
    let transactions = await $axios.$get(`/api/${tableNames.customers}/${params.id}/${tableNames.transactions}?order_by=transact_date`);
    const headers: any = [
      {
        field: 'transact_date',
        label: 'Date'
