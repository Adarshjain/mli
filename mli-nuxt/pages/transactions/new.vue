<template>
  <div class="mx-4 my-4" style="max-width: 500px; margin: auto">
    ...
    <b-field label="Select a customer*">
      <b-dropdown :mobile-modal="false">
        ...
        <b-dropdown-item
          v-for="customerName in customers"
          :key="customerName.id"
          @click="() => onCustomerSelect(customerName)"
        >
          {{ customerName.name }}
        </b-dropdown-item>
      </b-dropdown>
    ...
    <b-button :loading="isCreating" type="is-primary is-light" @click="clickMe">Create Project</b-button>
  </div>
</template>
...
export default class ProjectNew extends Vue {
  ...
  async asyncData({$axios}) {
    return {
      customers: await $axios.$get(`/api/${tableNames.customers}`)
    }
  }

  async onCustomerSelect(customerName) {
    this.customer = customerName;
    this.projects = [];
    this.project = '';
    //@ts-ignore
    this.projects = await this.$axios.$get(`/api/${tableNames.customers}/${this.customer.id}/${tableNames.projects}`)
  }
